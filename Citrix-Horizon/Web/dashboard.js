...
function generateUserReportScript() {
    ...
    lines.push('# Audit pull: tries server-side time filter on /external/v.../audit-events first (same idea as');
    lines.push('# App Report); optional Contains on user_name; falls back to unfiltered paging only if needed.');
    ...
    lines.push('}');
    lines.push('');
    lines.push('# --- Server-side filter JSON (Horizon 2506): "type" must be first property in each object ---');
    lines.push('$filterC = [ordered]@{ type = "And"; filters = @(');
    lines.push('    ([ordered]@{ type = "GreaterThanOrEqualTo"; name = "time"; value = $startMs }),');
    lines.push('    ([ordered]@{ type = "LessThanOrEqualTo";    name = "time"; value = $nowMs   })');
    lines.push('    )');
    lines.push('}');
    ...
    lines.push('$filterEJson = ($filterE | ConvertTo-Json -Depth 6 -Compress)');
    lines.push('$filterEEnc  = [System.Uri]::EscapeDataString($filterEJson)');
    lines.push('$filterB = [ordered]@{ type = "And"; filters = @(');
    lines.push('    ([ordered]@{ type = "GreaterThanOrEqualTo"; name = "eventTime"; value = $startMs }),');
    lines.push('    ([ordered]@{ type = "LessThanOrEqualTo";    name = "eventTime"; value = $nowMs   })');
    lines.push('    )');
    lines.push('}');
    lines.push('$filterBJson = ($filterB | ConvertTo-Json -Depth 6 -Compress)');
    lines.push('$filterBEnc  = [System.Uri]::EscapeDataString($filterBJson)');
    lines.push('# Prefer eventTime (B) before time (C): some controllers reject unknown filter field "time" with HTTP 400.');
    lines.push('$filterVariants = @(');
    lines.push('    @{ label = "B (eventTime range)";                 enc = $filterBEnc; json = $filterBJson },');
    lines.push('    @{ label = "C (time range, type first)";           enc = $filterCEnc; json = $filterCJson },');
    lines.push('    @{ label = "D (@type + time range)";              enc = $filterDEnc; json = $filterDJson },');
    lines.push('    @{ label = "E (@type time>=start only)";           enc = $filterEEnc; json = $filterEJson }');
    lines.push(')');
    ...
    lines.push('        @{ label = "F (time+Contains user_name)"; enc = $filterFEnc; json = $filterFJson },');
    lines.push('        @{ label = "G (time+Contains username)"; enc = $filterGEnc; json = $filterGJson }');
    lines.push('    )');
    lines.push('}');
    lines.push('');
