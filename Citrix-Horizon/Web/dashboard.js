...
function generateAppReportScript() {
    ...
    lines.push('# Default 40 pages x 250 = 10,000 newest events. Horizon returns newest-first;');
    lines.push('# raise this (e.g. 80, 200) if your 1-day window has more activity than fits here.');
    lines.push('$UnfilteredMaxPages = 40');
    lines.push('');
    ...
    lines.push("        '(?i)requested\\s+application\\s+([A-Za-z0-9][A-Za-z0-9 _\\.\\-\\+]{0,120}?)\\s+from\\b',");
    lines.push("        '(?i)has\\s+requested\\s+application\\s+\"([^\"]+)\"',");
