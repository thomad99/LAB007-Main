...
function generateHorizonAdminScript(action) {
    ...
    if (action === 'imageDates') {
        ...
        scriptLines.push('');
        scriptLines.push('# Attempt per-farm detail fetch to get base image / snapshot');
        ...
        scriptLines.push('');
        scriptLines.push('    # try detail match if still empty');
        scriptLines.push('    $fid = $farm.id; if (-not $fid) { $fid = $farm.farmId }');
        scriptLines.push('    if ($fid) {');
