...
function generateAppReportScript() {
    ...
    lines.push('# 10 pages x 250 events = 2500 events for a fast preview. Raise this if the preview');
    lines.push('# shows the right event types but the counts look too low for your window.');
    lines.push('$UnfilteredMaxPages = 10');
    lines.push('');
    ...
    lines.push('#   A) name="time"      + In on $launchEventTypes  (matches public REST spec)');
    lines.push('#   B) name="eventTime" + Or of Equals per type    (internal 2506 field names)');
    ...
    lines.push('}');
    lines.push('');
    lines.push('# --- Helper: unfiltered pull with SMALL batches + live progress + page cap ---');
    lines.push('function Get-HzAuditEventsUnfiltered($base, $hdrs, $epPath, $maxPages) {');
    lines.push('    $all = New-Object System.Collections.ArrayList');
    lines.push('    $page = 1');
    lines.push('    $size = 250');
    lines.push('    $start = Get-Date');
    lines.push('    if (-not $maxPages -or $maxPages -le 0) { $maxPages = 10 }');
    lines.push('    Write-Host ("  Unfiltered pull: page size {0}, max {1} pages (preview cap), progress every page." -f $size, $maxPages) -ForegroundColor DarkGray');
    lines.push('    while ($true) {');
    ...
    lines.push('            Write-Warning ("  Preview cap hit ({0} pages, {1} events). Raise `$UnfilteredMaxPages near the top of this script for a full pull." -f $maxPages, $all.Count)');
    lines.push('            break');
    lines.push('        }');
    lines.push('    }');
    lines.push('    return ,$all');
    ...
    lines.push('        $events = Get-HzAuditEventsUnfiltered -base $BaseUrl -hdrs $headers -epPath $p -maxPages $UnfilteredMaxPages');
    lines.push('        if ($events -and $events.Count -gt 0) {');
    lines.push('            $winningPath  = $p');
    lines.push('            $winningLabel = ("unfiltered (preview cap {0} pages)" -f $UnfilteredMaxPages)');
