...
function createCloneScript() {
    ...
    const script = generateCloneScript(selectedImages);
...
function generateCloneScript(selectedImages) {
    const scriptLines = [
    ...
    scriptLines.push('');
    scriptLines.push('# Ask user about moving source VMs after clone');
    scriptLines.push('$moveSourceVMs = $null');
    scriptLines.push('while ($moveSourceVMs -notmatch "^(y|n)$") {');
    scriptLines.push('    $moveSourceVMs = Read-Host "Move source VMs to backup folder after successful clone? (y/n) [n]"');
    scriptLines.push('    if ([string]::IsNullOrWhiteSpace($moveSourceVMs)) { $moveSourceVMs = "n" }');
    scriptLines.push('    $moveSourceVMs = $moveSourceVMs.ToLower()');
    scriptLines.push('}');
    scriptLines.push('$moveSourceVMs = $moveSourceVMs -eq "y"');
    scriptLines.push('');
    ...
    scriptLines.push('        # Get source VM folder for default selection');
    ...
    scriptLines.push('        ');
    scriptLines.push('        # Select destination folder for clone');
    scriptLines.push('        Write-Host ""');
    scriptLines.push('        Write-Host "  FOLDER SELECTION FOR CLONE:" -ForegroundColor Magenta');
    scriptLines.push('        Write-Host "  ===========================" -ForegroundColor Magenta');
    scriptLines.push('        $destinationFolder = Select-VMFolder -DefaultFolderPath $sourceVMFolder.Name');
    scriptLines.push('        Write-Host ""');
    scriptLines.push('        ');
    scriptLines.push('        # Select backup folder for source VM if requested');
    scriptLines.push('        $backupFolder = $null');
    scriptLines.push('        if ($moveSourceVMs) {');
    scriptLines.push('            Write-Host "  BACKUP FOLDER SELECTION:" -ForegroundColor Magenta');
    scriptLines.push('            Write-Host "  ========================" -ForegroundColor Magenta');
    scriptLines.push('            $backupFolder = Select-VMFolder -DefaultFolderPath "Backup"');
    scriptLines.push('            Write-Host ""');
    scriptLines.push('        }');
    scriptLines.push('        ');
