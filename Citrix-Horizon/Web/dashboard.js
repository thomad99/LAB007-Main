...
function generateAppReportScript() {
    ...
    lines.push('    )');
    lines.push('}');
    lines.push('$filterAEnc = [System.Uri]::EscapeDataString( ($filterA | ConvertTo-Json -Depth 6 -Compress) )');
    lines.push('');
    lines.push('# Variant B');
    lines.push('$typeEqualsFilters = @()');
    lines.push('foreach ($t in $launchEventTypes) {');
    lines.push('    $typeEqualsFilters += @{ type = "Equals"; name = "type"; value = $t }');
    lines.push('}');
    lines.push('$filterB = @{');
    lines.push('    type    = "And"');
    lines.push('    filters = @(');
    lines.push('        @{ type = "GreaterThanOrEqualTo"; name = "eventTime"; value = $startMs },');
    lines.push('        @{ type = "LessThanOrEqualTo";    name = "eventTime"; value = $nowMs   },');
    lines.push('        @{ type = "Or"; filters = $typeEqualsFilters }');
    lines.push('    )');
    lines.push('}');
    lines.push('$filterBEnc = [System.Uri]::EscapeDataString( ($filterB | ConvertTo-Json -Depth 6 -Compress) )');
    lines.push('');
    ...
    lines.push('    )');
    lines.push('}');
    lines.push('$filterCEnc = [System.Uri]::EscapeDataString( ($filterC | ConvertTo-Json -Depth 6 -Compress) )');
    lines.push('');
    lines.push('$filterVariants = @(');
    lines.push('    @{ label = "A (time+In on type)";              enc = $filterAEnc },');
    lines.push('    @{ label = "B (eventTime+Or of Equals)";       enc = $filterBEnc },');
    lines.push('    @{ label = "C (time range only, no type)";     enc = $filterCEnc }');
    lines.push(')');
    lines.push('');
    ...
    lines.push('        $try = Get-HzAuditEvents -base $BaseUrl -hdrs $headers -epPath $p -filterEnc $fv.enc');
    ...
    lines.push('            $winningPath  = $p');
    lines.push('            $winningLabel = "unfiltered"');
