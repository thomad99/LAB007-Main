...
function generateHorizonAdminScript(action) {
    ...
    if (action === 'imageDates') {
        ...
        scriptLines.push('    try {');
        scriptLines.push('        $r = Invoke-RestMethod -Method Get -Uri $uri -Headers $headers');
        scriptLines.push('        if ($r -is [hashtable] -and $r.ContainsKey("items")) { $farmData = $r.items }');
    ...
    scriptLines.push('');
    if (action === 'imageDates') {
        scriptLines.push('# Transform for image dates view (source: inventory/v7 or v4 farms + per-farm detail)');
        ...
        scriptLines.push('    $fid = $farm.id; if (-not $fid) { $fid = $farm.farmId }');
        scriptLines.push('');
        scriptLines.push('    # VMware Horizon inventory: automated_farm_settings.provisioning_settings (List Farms V4/V7)');
        ...
        scriptLines.push('# Transform clones: merge tasks + farm provisioning_status_data (inventory)');
        ...
        scriptLines.push('if ($farms -is [pscustomobject] -and $farms.PSObject.Properties.Name -contains "items") { $farms = $farms.items }');
        scriptLines.push('if ($farms -isnot [array]) { $farms = @($farms) }');
        scriptLines.push('');
        scriptLines.push('$rows = @()');
        scriptLines.push('# Task rows');
        scriptLines.push('foreach ($t in $tasks) {');
        scriptLines.push('    $rows += [PSCustomObject]@{');
        scriptLines.push('        Type     = "Task"');
        scriptLines.push('        Id       = $t.id');
        scriptLines.push('        Name     = $t.name');
        scriptLines.push('        Status   = $t.status');
        scriptLines.push('        Progress = $t.progress');
        scriptLines.push('        FarmId   = $t.farm_id');
        scriptLines.push('        Error    = $t.error_message');
        scriptLines.push('    }');
        scriptLines.push('}');
        scriptLines.push('# Farm provisioning status (inventory provisioning_status_data)');
        scriptLines.push('foreach ($f in $farms) {');
        scriptLines.push('    $pst = $f.provisioning_status_data');
        scriptLines.push('    if (-not $pst) { continue }');
        scriptLines.push('    $op = $pst.instant_clone_operation');
        scriptLines.push('    $err = $pst.last_provisioning_error');
        scriptLines.push('    $parent = $pst.instant_clone_pending_image_parent_vm_id; if (-not $parent) { $parent = $pst.parent_vm_id }');
        scriptLines.push('    $snap = $pst.instant_clone_pending_image_snapshot_id; if (-not $snap) { $snap = $pst.base_snapshot_id }');
        scriptLines.push('    $rows += [PSCustomObject]@{');
        scriptLines.push('        Type     = "FarmProvisioning"');
        scriptLines.push('        Id       = $f.id');
        scriptLines.push('        Name     = $f.name');
        scriptLines.push('        Status   = $op');
        scriptLines.push('        Progress = ""');
        scriptLines.push('        FarmId   = $f.id');
        scriptLines.push('        Error    = $err');
        scriptLines.push('        ParentVm = $parent');
        scriptLines.push('        Snapshot = $snap');
        scriptLines.push('    }');
        scriptLines.push('}');
        scriptLines.push('$response = $rows');
    }
