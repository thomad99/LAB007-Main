...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    const enableVMwareFoldersToggle = document.getElementById('enableVMwareFoldersToggle');
    if (enableVMwareFoldersToggle) {
        enableVMwareFoldersToggle.addEventListener('change', function() {
            const isEnabled = this.checked;
            const vmwareFoldersSection = document.getElementById('vmwareFoldersSection');

            if (vmwareFoldersSection) {
                if (isEnabled) {
                    vmwareFoldersSection.style.display = 'block';
                } else {
                    vmwareFoldersSection.style.display = 'none';
                }
            }
        });

        // Initialize toggle state (default to enabled)
        enableVMwareFoldersToggle.checked = true;
        enableVMwareFoldersToggle.dispatchEvent(new Event('change'));
    }
...
function createCloneScript() {
    ...
    // Get VMware folder operations toggle state
    const enableVMwareFolders = document.getElementById('enableVMwareFoldersToggle').checked;

    // Get folder and move options (only if VMware folders are enabled)
    ...
    if (enableVMwareFolders && !destinationFolder) {
        alert('Please specify a destination folder for the cloned VMs.');
        return;
    }

    if (enableVMwareFolders && moveSourceAfterClone && !sourceMoveFolder) {
...
function generateCloneScript(selectedImages, destinationFolder, moveSourceAfterClone, sourceMoveFolder, enableVMwareFolders) {
    const scriptLines = [
    ...
    if (enableVMwareFolders) {
        scriptLines.push('$moveSourceVMs = ' + (moveSourceAfterClone ? '$true' : '$false'));
        scriptLines.push('$destinationFolderName = "' + destinationFolder.replace(/"/g, '""') + '"');
        if (moveSourceAfterClone) {
    ...
    scriptLines.push('        # Get destination folder from web interface configuration');
    if (enableVMwareFolders) {
        scriptLines.push('        if ($enableVMwareFolders) {');
    ...
    if (enableVMwareFolders) {
        scriptLines.push('        if ($enableVMwareFolders -and $moveSourceVMs) {');
