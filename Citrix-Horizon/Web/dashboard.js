...
function generateAppReportScript() {
    ...
    lines.push('}');
    lines.push('');
    lines.push('# --- Helper: unfiltered pull with SMALL batches + live progress (last-resort fallback) ---');
    lines.push('function Get-HzAuditEventsUnfiltered($base, $hdrs, $epPath) {');
    lines.push('    $all = New-Object System.Collections.ArrayList');
    lines.push('    $page = 1');
    lines.push('    $size = 250');
    lines.push('    $start = Get-Date');
    lines.push('    Write-Host ("  Unfiltered pull: page size {0}, max 2000 pages, progress every page." -f $size) -ForegroundColor DarkGray');
    lines.push('    while ($true) {');
    ...
    lines.push('        }');
    lines.push('        $page++');
    lines.push('        if ($page -gt 2000) { Write-Warning "  Page guard hit (2000) on $epPath - truncating."; break }');
    lines.push('    }');
    lines.push('    return ,$all');
    lines.push('}');
    lines.push('');
