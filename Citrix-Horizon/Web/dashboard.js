...
function generateUserReportScript() {
    ...
    lines.push('Write-Host "  Source: Horizon External API GET /external/v1/audit-events or /external/v2/audit-events (not /main/events)." -ForegroundColor DarkGray');
    lines.push('$events = @(); $auditEp = $null');
    lines.push('foreach ($p in @("/external/v1/audit-events","/external/v2/audit-events")) {');
    lines.push('    Write-Host ("  Trying audit endpoint {0} ..." -f $p) -ForegroundColor Cyan');
    ...
    lines.push('    if ($try -and $try.Count -gt 0) { $events = $try; $auditEp = $p; Write-Host ("  Using {0} — loaded {1} audit events into memory." -f $p, $events.Count) -ForegroundColor Green; break }');
    lines.push('    else { Write-Host ("  {0}: returned 0 events; trying next endpoint if available." -f $p) -ForegroundColor Yellow }');
    lines.push('}');
    lines.push('if (-not $events) { $events = @() }');
    lines.push('Write-Host ("Total audit events in memory (before user filter): {0}" -f $events.Count) -ForegroundColor Cyan');
    lines.push('');
    lines.push('function Unpack-Items($r) {');
    lines.push('    if ($null -eq $r) { return @() }');
    lines.push('    if ($r -is [System.Array]) { return $r }');
