...
function generateUserReportScript() {
    ...
    lines.push('}');
    lines.push('');
    lines.push('function Get-HzAuditEventsFiltered($base, $hdrs, $epPath, $filterEnc, $filterLabel) {');
    lines.push('    $all = New-Object System.Collections.ArrayList');
    lines.push('    $page = 1; $size = 500; $t0 = Get-Date');
    lines.push('    while ($true) {');
    ...
    lines.push('    )');
    lines.push('}');
    lines.push('$filterCJson = ($filterC | ConvertTo-Json -Depth 8 -Compress)');
    lines.push('$filterCEnc  = [System.Uri]::EscapeDataString($filterCJson)');
    lines.push('$filterD = [ordered]@{');
    lines.push("    '@type' = 'And'");
    lines.push('    filters = @(');
    ...
    lines.push('    )');
    lines.push('}');
    lines.push('$filterDJson = ($filterD | ConvertTo-Json -Depth 8 -Compress)');
    lines.push('$filterDEnc  = [System.Uri]::EscapeDataString($filterDJson)');
    lines.push('$filterE = [ordered]@{');
    lines.push("    '@type' = 'GreaterThanOrEqualTo'; 'name' = 'time'; 'value' = $startMs");
    lines.push('}');
    lines.push('$filterEJson = ($filterE | ConvertTo-Json -Depth 8 -Compress)');
    lines.push('$filterEEnc  = [System.Uri]::EscapeDataString($filterEJson)');
    ...
    lines.push('    )');
    lines.push('}');
    lines.push('$filterBJson = ($filterB | ConvertTo-Json -Depth 8 -Compress)');
    lines.push('$filterBEnc  = [System.Uri]::EscapeDataString($filterBJson)');
    lines.push('$filterVariants = @(');
    ...
    lines.push('    )');
    lines.push('    }');
    lines.push('    $filterFJson = ($filterF | ConvertTo-Json -Depth 8 -Compress)');
    ...
    lines.push('    )');
    lines.push('    }');
    lines.push('    $filterGJson = ($filterG | ConvertTo-Json -Depth 8 -Compress)');
    ...
    lines.push('        $try = Get-HzAuditEventsFiltered -base $BaseUrl -hdrs $headers -epPath $p -filterEnc $fv.enc -filterLabel $fv.label');
    lines.push('        if ($try -and $try.Count -gt 0) {');
