...
async function scrapeHoursFromPage(page) {
  // Open first Maps result when still on a search list
  try {
    await page.evaluate(() => {
      const link =
        document.querySelector('a[href*="/maps/place/"]') ||
        document.querySelector('[role="feed"] a[href*="/maps/place/"]') ||
        document.querySelector('div[role="article"] a');
      if (link) link.click();
    });
    await sleep(2000);
  } catch (_) {
  ...
  try {
    await page.evaluate(() => {
      ...
      const hit = nodes.find((el) => {
        ...
        return /^(hours|opening hours|see more hours|show.*hours)\b/i.test(label) || /^hours$/i.test(label);
      });
      if (hit) hit.click();
    });
    await sleep(1200);
  ...
  // Wait for weekday text if possible
  try {
    if (typeof page.waitForFunction === 'function') {
      await page.waitForFunction(
        () => /Monday|Tuesday|Wednesday/i.test((document.body && document.body.innerText) || ''),
        { timeout: 12000 }
      );
    } else {
      await sleep(3000);
  ...
  return page.evaluate(() => {
    ...
    const looksLikeHours = (s) =>
      /closed|24\s*hours|\d{1,2}(?::\d{2})?\s*[ap]m|\d{1,2}\s*[–—−-]\s*\d{1,2}/i.test(s);

    const descriptions = [];
    const seen = new Set();
    const add = (day, rest) => {
      const r = normalize(rest);
      if (!day || !r || !looksLikeHours(r)) return;
      const key = day.toLowerCase();
      if (seen.has(key)) return;
      seen.add(key);
      descriptions.push(`${day} ${r}`);
    };

    // aria-labels: "Monday, 9 AM to 5 PM"
    Array.from(document.querySelectorAll('[aria-label]')).forEach((el) => {
      const label = normalize(el.getAttribute('aria-label'));
      const day = dayNames.find((d) => new RegExp(`^${d}\\b`, 'i').test(label));
      if (!day) return;
      add(day, label.replace(new RegExp(`^${day}\\s*[,:]?\\s*`, 'i'), ''));
    });

    // table rows
    Array.from(document.querySelectorAll('table tr, [role="row"]')).forEach((row) => {
      const cells = Array.from(row.querySelectorAll('td, th, [role="cell"], div, span'))
        .map((c) => normalize(c.textContent))
        .filter(Boolean);
      if (cells.length < 2) return;
      const day = dayNames.find((d) => cells.some((c) => c === d || c.startsWith(d + ' ')));
      if (!day) return;
      const rest = cells.find((c) => c !== day && looksLikeHours(c));
      if (rest) add(day, rest);
    });
    ...
    for (let i = 0; i < lines.length; i += 1) {
      ...
      if (!rest || !looksLikeHours(rest)) {
        for (let j = 1; j <= 3; j += 1) {
          const next = lines[i + j] || '';
          if (dayNames.includes(next)) break;
          if (looksLikeHours(next)) {
            rest = next;
            break;
          }
        }
    ...
    title = title
      .replace(/\s*[-–|].*$/, '')
      .replace(/Google Maps|Google Search/gi, '')
      .trim();
    const h1 = document.querySelector('h1');
    let name = (h1 && h1.textContent ? normalize(h1.textContent) : '') || title;
    if (!name || /^https?:\/\//i.test(name) || /google\./i.test(name)) {
      const og = document.querySelector('meta[property="og:title"]');
      name = normalize(og && og.getAttribute('content')) || name;
    }
    ...
    return {
      ...
      url: location.href,
      descriptions,
      sample: lines.filter((l) => /Monday|Tuesday|AM|PM|Closed|Hours|Open/i.test(l)).slice(0, 50)
    };
  });
}
  ...
  try {
    ...
    for (const target of targets) {
      try {
        ...
        await sleep(2500);
        const scraped = await scrapeHoursFromPage(page);
        lastScraped = scraped;
        ...
        console.warn('[SmartHours] scrape target failed:', target, err.message);
      }
    }

    const label = friendlyBusinessLabel(resolved, lastScraped);
    const err = new Error(
      `Could not read opening hours from Google for “${label}”. Open the share link in a browser to confirm hours are public, or set GOOGLE_PLACES_API_KEY for reliable sync.`
    );
    err.status = 422;
