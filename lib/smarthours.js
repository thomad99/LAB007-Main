...
function defaultLayout() {
  return [
    ...
    { id: 'slideshow', type: 'slideshow', x: 40, y: 340, w: 520, h: 280 },
...
function normalizeLayout(raw) {
  ...
  if (Array.isArray(raw)) {
    raw.forEach((item) => {
      if (item && item.type) byType.set(String(item.type), item);
    });
  }
...
function normalizeDesign(raw) {
  ...
  return {
    ...
    showSlideshow: Boolean(src.showSlideshow),
    statusMode,
...
async function fetchGooglePlaceHours(placeIdOrUrl) {
  ...
  if (!periods.length && Array.isArray(opening.weekdayDescriptions) && opening.weekdayDescriptions.length) {
    // Still return closed-default hours but keep descriptions for debugging
  }
  const hours = periodsToHours(periods);
  const hasAnyOpen = DAYS.some((day) => hours[day] && !hours[day].closed);
  if (!hasAnyOpen && !periods.length) {
    const err = new Error(
