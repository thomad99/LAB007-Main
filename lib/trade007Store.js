...
function trade007StatePath() {
  ...
  // Render persistent disk default (if mounted and no explicit path provided).
  const renderDisk = '/var/data';
  if (fs.existsSync(renderDisk)) {
    return path.join(renderDisk, 'lab007', 'Trade007', 'state.json');
  }
...
function defaultAutomationState() {
  return {
    ...
    /** If true, automation scales maxAccountRiskPct by current market regime multiplier. */
    marketConditionSizingEnabled: false,
    marketConditionRiskFloor: 0.35,
    marketConditionRiskCeiling: 1.1,
    /** 0 = off. Block new entries when open position market value sum exceeds this. */
    maxTotalExposureUsd: 0,
    /** 0 = off. Block new entries when a symbol's absolute market value exceeds this. */
    maxPerSymbolExposureUsd: 0,
    /** 0 = off. Max number of automation entries per ET day. */
    maxAutoEntriesPerDay: 0,
    lastMarketType: null,
...
function defaultState() {
  return {
    ...
    /** User-maintained strategy knowledge notes and imported references */
    strategyKnowledgeEntries: [],
    /** Append-only log of orders placed via this app (for per-strategy reporting) */
    tradeJournal: [],
    copyTrader: {
      enabled: false,
      mode: 'shadow', // shadow | semi_auto
      maxAllocationPct: 20,
      minTrackRecordMonths: 12,
      maxDrawdownPct: 12,
      minSharpe: 1.2,
      notes: ''
    },
      ...
      activeStrategyIds:
        Array.isArray(j.activeStrategyIds) && j.activeStrategyIds.length > 0
          ...
          : j.selectedStrategyId
            ? [j.selectedStrategyId]
            : ['momentum-breakout'],
      strategyKnowledgeEntries: Array.isArray(j.strategyKnowledgeEntries)
        ? j.strategyKnowledgeEntries.slice(0, 200)
        : [],
      tradeJournal: Array.isArray(j.tradeJournal) ? j.tradeJournal.slice(0, 500) : [],
      copyTrader: {
        ...defaultState().copyTrader,
        ...(j.copyTrader && typeof j.copyTrader === 'object' ? j.copyTrader : {})
      },
      automation: (() => {
...
function writeState(state) {
  ...
  // Atomic write guards against partial/corrupt state during abrupt restarts.
  const tmp = `${p}.tmp`;
