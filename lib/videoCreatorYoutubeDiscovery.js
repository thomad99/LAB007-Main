...
/**
 * Google Cloud API keys restricted to "HTTP referrers" only work from browsers.
 * Server-side Node requests have no Referer → Google returns forbidden / referer blocked.
 */
function hintForYoutubeDiscoveryError(message) {
  const m = String(message || '');
  const low = m.toLowerCase();
  if (
    /referer|referrer/i.test(m) ||
    (/blocked/i.test(low) && /request/i.test(low)) ||
    /^forbidden\b/i.test(low.trim())
  ) {
    return (
      'Your API key is restricted to HTTP referrers (browser-only). This app calls YouTube from your server, so there is no Referer. In Google Cloud Console → APIs & Services → Credentials → edit this key: under Application restrictions choose IP addresses (your server’s public IP) or None for local testing — not HTTP referrers. Ensure API restrictions include YouTube Data API v3.'
    );
  }
  return null;
}
...
function sanitizeSearchQuery(q) {
  ...
  let s = String(q).trim().slice(0, 160);
  s = s.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '');
  return s.trim();
}

/**
 * One search.list page. Supports explicit topic `q`, category-only (+ optional fallback `q`).
 */
async function searchListPage(fetchFn, apiKey, opts) {
  ...
  const explicitQ = sanitizeSearchQuery(opts.searchQuery);

  const buildUrl = (qValOrNull) => {
    ...
    if (qValOrNull != null && String(qValOrNull).trim()) {
      u.searchParams.set('q', String(qValOrNull).trim().slice(0, 100));
    }
  ...
  if (explicitQ) {
    ...
    url = buildUrl(String(opts.anyQuery || 'interesting').trim() || 'interesting');
    ({ ok, j } = await readJson(fetchFn, url));
    if (!ok) {
      const err = new Error(youtubeErrorMessage(j));
      err.status = j && j.error && j.error.code ? Number(j.error.code) : 502;
 ...
 * @param {{ categoryId?: string, range: string, searchQuery?: string, regionCode?: string, maxVideos?: number }} params
 */
async function discoverYoutubeVideos(fetchFn, params) {
  ...
  const searchUsed = sanitizeSearchQuery(params.searchQuery);
  ...
  if (!idsUnique.length) {
    return {
      ...
      searchQuery: searchUsed || undefined,
      range,
      quotaNote:
  ...
  const clamp =
    typeof params.maxVideos === 'number' && Number.isFinite(params.maxVideos)
      ? Math.min(25, Math.max(3, params.maxVideos))
      : 10;
