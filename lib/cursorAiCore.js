...
function cursorAiAnthropicMaxTokens() {
  const n = parseInt(process.env.CURSORAI_ANTHROPIC_MAX_TOKENS || '', 10);
  if (Number.isFinite(n) && n >= 1024 && n <= 32000) return n;
  return 8192;
}

function cursorAiOpenAiMaxTokens() {
  const n = parseInt(process.env.CURSORAI_OPENAI_MAX_TOKENS || '', 10);
  if (Number.isFinite(n) && n >= 512 && n <= 16384) return n;
  return 8192;
}

function getCursorAiJsonRepair() {
  try {
    return require('jsonrepair').jsonrepair;
  } catch {
    return null;
  }
}

function parseCursorAiModelJson(text, meta = {}) {
  ...
  const slice =
    clean.indexOf('{') >= 0 && clean.lastIndexOf('}') > clean.indexOf('{')
      ? clean.slice(clean.indexOf('{'), clean.lastIndexOf('}') + 1)
      : clean;

  const repair = getCursorAiJsonRepair();
  const attempts = [];
  for (const body of slice === clean ? [clean] : [clean, slice]) {
    attempts.push(body);
    if (repair) attempts.push(repair(body));
  }
  ...
  const hint = meta.truncated
    ? ' Model output may have been truncated (try CURSORAI_ANTHROPIC_MAX_TOKENS or a shorter prompt).'
    : '';
  throw new Error(
    `${lastErr?.message || 'JSON parse failed'}.${hint} Ensure deploy ran npm install (jsonrepair).`
  );
}
...
- CRITICAL: The JSON must be strictly valid. Inside every "content" string, escape double quotes as \\", backslashes as \\\\, and use \\n for newlines (never paste raw multi-line text into a JSON string).
- Prefer a compact but working implementation if the app is large; avoid huge inline comments or redundant whitespace in file contents.
`;
  ...
  if (picked === 'openai') {
    ...
    const r = await fetchFn('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.2,
        max_tokens: cursorAiOpenAiMaxTokens(),
        response_format: { type: 'json_object' },
        messages: [
          {
            role: 'system',
            content:
              'You are a careful web code generator for LAB007. Follow the user request and every LAB007 default design rule in the prompt. Always respond with a single valid JSON object only (no markdown).'
          },
    ...
    const r = await fetchFn('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: cursorAiAnthropicMaxTokens(),
        messages: [{ role: 'user', content: buildPrompt }]
      })
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j?.error?.message || `Anthropic request failed (${r.status})`);
    if (String(j.stop_reason || '') === 'max_tokens') {
      throw new Error(
        'Model hit max output length (JSON was truncated). Set CURSORAI_ANTHROPIC_MAX_TOKENS higher on the server, or use a shorter / simpler prompt.'
      );
    }
  ...
  let parsed = null;
  try {
    parsed = parseCursorAiModelJson(text, { truncated: false });
  } catch (e) {
    throw new Error(e.message || 'Could not parse model JSON output.');
  }
  if (!parsed || !Array.isArray(parsed.files) || !parsed.files.length) {
    const rawFallback = String(text || '').trim();
    return {
      ...
      files: [
        {
          path: 'index.html',
          content:
            rawFallback || '<!doctype html><title>CursorAI</title><h1>No output</h1>'
        }
      ]
    };
  }
