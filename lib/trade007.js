...
function normalizeStrategyKnowledgeEntries(entries) {
  if (!Array.isArray(entries)) return [];
  return entries
    .filter((x) => x && typeof x === 'object')
    .map((x) => ({
      id: String(x.id || '').trim() || crypto.randomUUID(),
      title: String(x.title || 'Untitled strategy note').trim().slice(0, 120),
      sourceType: x.sourceType === 'url' ? 'url' : 'text',
...
async function alpacaFetchFillActivities(limit = 300) {
  if (!alpacaConfigured()) return [];
  const base = alpacaBaseUrl();
  const h = alpacaHeaders();
  const pageSize = Math.min(500, Math.max(1, parseInt(limit, 10) || 300));
  const r = await fetchFn(`${base}/v2/account/activities/FILL?direction=desc&page_size=${pageSize}`, {
    headers: h
  });
  const j = await r.json().catch(() => []);
  if (!r.ok) throw new Error((j && (j.message || j.error)) || `FILL activities ${r.status}`);
...
function registerTrade007Routes(app) {
  ...
  app.get('/007trade/strategy', (req, res) => {
    const p = path.join(__dirname, '..', 'public', '007trade-strategy.html');
    if (fs.existsSync(p)) return res.sendFile(p);
    return res.status(404).send('Not found');
  });
  ...
  app.get('/007trade/reports', (req, res) => {
    const p = path.join(__dirname, '..', 'public', '007trade-reports.html');
    if (fs.existsSync(p)) return res.sendFile(p);
    return res.status(404).send('Not found');
  });

  app.get('/007trade/logs', (req, res) => {
    const p = path.join(__dirname, '..', 'public', '007trade-logs.html');
    if (fs.existsSync(p)) return res.sendFile(p);
    return res.status(404).send('Not found');
  });
  ...
  app.get('/api/007trade/alpaca-dashboard', async (req, res) => {
    ...
    try {
      ...
      const ordListRaw = Array.isArray(ordersJson) ? ordersJson : [];
      const stForMeta = readState();
      const journal = Array.isArray(stForMeta.tradeJournal) ? stForMeta.tradeJournal : [];
      const orderMeta = new Map();
      for (const row of journal) {
        const oid = String(row.alpacaOrderId || '').trim();
        if (!oid || orderMeta.has(oid)) continue;
        orderMeta.set(oid, {
          source: row.source || null,
          ruleId: row.ruleId || null,
          strategyName: row.strategyName || row.strategyId || null,
          reason: row.reason || null
        });
      }
      const ordList = ordListRaw.map((o) => {
        const meta = orderMeta.get(String(o.id || '').trim()) || null;
        return { ...o, _meta: meta };
      });
  ...
  app.get('/api/007trade/strategy-knowledge', (req, res) => {
    const st = readState();
    const entries = normalizeStrategyKnowledgeEntries(st.strategyKnowledgeEntries);
    res.json({ ok: true, builtins: BUILTIN_STRATEGIES, entries });
  });

  app.post('/api/007trade/strategy-knowledge', async (req, res) => {
    try {
      const st = readState();
      const body = req.body || {};
      ...
      if (sourceType === 'url') {
        ...
        if (!r.ok) {
          return res.status(400).json({ ok: false, error: `Unable to read URL (HTTP ${r.status})` });
        }
        const html = await r.text();
        content = stripHtmlToText(html).slice(0, 12000);
      }

      if (!content) {
        return res.status(400).json({ ok: false, error: 'Strategy content is required.' });
      }

      const entry = {
        id: crypto.randomUUID(),
        title: title || (sourceType === 'url' ? 'Strategy note from URL' : 'Strategy note'),
        sourceType,
        url: sourceType === 'url' ? url : '',
        content,
        createdAt: new Date().toISOString()
      };
      const current = normalizeStrategyKnowledgeEntries(st.strategyKnowledgeEntries);
      st.strategyKnowledgeEntries = [entry, ...current].slice(0, 200);
      writeState(st);
      return res.json({ ok: true, entry, entries: st.strategyKnowledgeEntries });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message || 'Failed to save strategy knowledge.' });
    }
  });
  ...
  app.get('/api/007trade/report', async (req, res) => {
    const st = readState();
    ...
    const enabledRules = Array.isArray(auto.rules) ? auto.rules.filter((r) => r && r.enabled).length : 0;
    const todayEt = new Date().toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
    const completedToday = journal.filter((row) => {
      if (row.source !== 'automation' || !row.ts) return false;
      const d = new Date(row.ts).toLocaleDateString('en-CA', { timeZone: 'America/New_York' });
      return d === todayEt;
    }).length;
    const autoState =
      auto.masterEnabled && enabledRules > 0 && !(auto.daily && auto.daily.halted) ? 'Active' : 'Waiting';
    ...
    try {
      const orderToStrategy = new Map();
      for (const row of journal) {
        if (row.alpacaOrderId) orderToStrategy.set(String(row.alpacaOrderId), String(row.strategyId || 'unknown'));
      }
      const fills = await alpacaFetchFillActivities(400);
      for (const f of fills) {
        const orderId = String(f.order_id || '');
        const sid = orderToStrategy.get(orderId);
        if (!sid) continue;
        ...
        const signed = side === 'sell' ? qty * px : -qty * px;
        const row = summary.find((x) => x.strategyId === sid);
        if (row) row.actionPnlUsd += signed;
      }
    } catch (e) {
      // Keep report usable even if broker activities are unavailable.
    }

    res.json({
      ok: true,
      disclaimer:
        'Action P&L is computed from Alpaca FILL activities mapped to 007Trade-tagged order IDs (cashflow style: sells positive, buys negative).',
      totals: {
  ...
  app.get('/api/007trade/logs', (req, res) => {
    const st = readState();
    const auto = st.automation && typeof st.automation === 'object' ? st.automation : {};
    const autoLog = Array.isArray(auto.log) ? auto.log.slice(0, 300) : [];
    const journal = Array.isArray(st.tradeJournal) ? st.tradeJournal.slice(0, 500) : [];
    res.json({
      ok: true,
      logs: {
        automation: autoLog,
  ...
  const { startTrade007Automation, runAutomationTick } = require('./trade007Automation');
  ...
  app.post('/api/007trade/automation/run-now', async (req, res) => {
    try {
      if (!alpacaConfigured()) {
        return res.status(400).json({ ok: false, error: 'Alpaca keys not configured.' });
      }
      const st = readState();
      const auto = st.automation && typeof st.automation === 'object' ? st.automation : defaultAutomationState();
      if (!auto.masterEnabled) {
        return res.status(409).json({
          ok: false,
          error: 'Master automation is disabled. Enable it on Circuit Breaker first.'
        });
      }
      if (!alpacaIsPaper() && !auto.allowLiveAutomation) {
        return res.status(409).json({
          ok: false,
          error: 'Automation is blocked in live mode until "Allow automation in live mode" is enabled.'
        });
      }

      await runAutomationTick(automationDeps);
      const fresh = readState();
      const nextAuto =
        fresh.automation && typeof fresh.automation === 'object' ? fresh.automation : defaultAutomationState();
      return res.json({
        ok: true,
        ranAt: new Date().toISOString(),
        lastTickAt: nextAuto.lastTickAt || null,
        dryRun: Boolean(nextAuto.dryRun),
        enabledRules: Array.isArray(nextAuto.rules) ? nextAuto.rules.filter((r) => r && r.enabled).length : 0,
        halted: Boolean(nextAuto.daily && nextAuto.daily.halted)
      });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message || 'Automation run failed.' });
    }
  });

  startTrade007Automation(automationDeps);
}
...
function sanitizeAutomationRules(rules) {
  ...
  for (const r of rules) {
    ...
    const sizeModeRaw = String(r.sizeMode || '').trim().toLowerCase();
    const sizeMode = sizeModeRaw === 'fixed' ? 'fixed' : 'auto';
    const fixedQty = Math.min(100000, Math.max(1, parseInt(r.fixedQty, 10) || 1));
    out.push({
      id,
