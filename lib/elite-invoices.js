...
async function buildInvoicePdf(invoice) {
  ...
  if (process.env.NODE_ENV !== 'production') {
    console.warn('[EliteInvoices] HTML renderer unavailable, using plain-text fallback PDF.');
    return buildInvoicePdfFallback(invoice);
  }

  throw new Error(
    'Branded invoice PDF could not be generated. Chromium is not available on the server. ' +
    'Redeploy after browsers install, or check Render logs for Playwright/Puppeteer errors.'
  );
}
