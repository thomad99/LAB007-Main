...
function buildInvoicePdfWithPdfKit(invoice) {
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.text(description, left + 8, y + 8, { width: colDesc - 8, lineBreak: false });
...
function buildInvoicePdfFallback(invoice) {
  ...
  lines.push(`${description.padEnd(40)} 1   ${formatMoney(invoice.amount).padStart(6)}   ${formatMoney(invoice.amount).padStart(6)}`);
  lines.push('');
