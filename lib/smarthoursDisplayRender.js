...
function smartHoursFontFaceCss() {
  ...
  if (regular) {
    css += `@font-face{font-family:"SmartHours";src:url(data:font/woff2;base64,${regular}) format("woff2");font-weight:400 900;font-style:normal;font-display:block}`;
  }
  return css;
}
  ...
  const elements = (design.layout || [])
    .map((el, index) => {
      ...
      if (el.type === 'logo' && logoUrl) {
        ...
        const url = fileToDataUrl(file);
        if (url) {
          inner = `<img class="custom-img" style="object-fit:contain;background:transparent" src="${url}" alt="">`;
        }
      }
      if (!inner) return '';
