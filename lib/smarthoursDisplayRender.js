...
function smartHoursFontFaceCss() {
  const fontsDir = path.join(__dirname, '..', 'public', 'smarthours-assets', 'fonts');
  const readB64 = (name) => {
    try {
      return fs.readFileSync(path.join(fontsDir, name)).toString('base64');
    } catch (_) {
      return '';
    }
  };
  const light = readB64('SmartHours-Light.woff2');
  const regular = readB64('SmartHours-Regular.woff2');
  let css = '';
  if (light) {
    css += `@font-face{font-family:"SmartHours";src:url(data:font/woff2;base64,${light}) format("woff2");font-weight:300;font-style:normal;font-display:block}`;
  }
  if (regular) {
    css += `@font-face{font-family:"SmartHours";src:url(data:font/woff2;base64,${regular}) format("woff2");font-weight:400;font-style:normal;font-display:block}`;
    css += `@font-face{font-family:"SmartHours";src:url(data:font/woff2;base64,${regular}) format("woff2");font-weight:700;font-style:normal;font-display:block}`;
  }
  return css;
}

function layoutFontWeight(el, family) {
  const bold = !(el && el.bold === false);
  if (/SmartHours/i.test(String(family || ''))) return bold ? '400' : '300';
  return bold ? '700' : '400';
}
...
function symbolGlyphHtml(url, color) {
  const c = /^#([0-9a-fA-F]{6})$/.test(String(color || '').trim()) ? String(color).trim() : '#000000';
  const u = cssMaskUrl(url);
  return `<div class="symbol-glyph" style="background-color:${escapeHtml(c)};-webkit-mask-image:url('${u}');mask-image:url('${u}')"></div>`;
}
...
function displayContentVersion(customer) {
  ...
  const payload = JSON.stringify({
    ...
    design,
    eink,
    render: 'display-png-v9-hang-buttons-up'
  });
...
function buildDisplayHtml(customer, dataDir) {
  ...
  const fileToUrl = (id) => fileToDataUrl(resolveImageFile(dataDir, customer, id));
  const openImg = fileToUrl(design.openMediaId);
  const closedImg = fileToUrl(design.closedMediaId);
  const slideUrls = (design.slideshowMediaIds || []).map(fileToUrl).filter(Boolean);
  ...
  const font = design.fontFamily || '"SmartHours", sans-serif';
  ...
  const statusHtml = (() => {
    ...
    const colors = resolveStatusBadgeColors(statusEl, design, openNow);
    return `<div class="status-badge" style="font-size:${statusSize}px;font-family:${escapeHtml(statusFont)};font-weight:${layoutFontWeight(statusEl, statusFont)};background:${
      colors.box
    };color:${colors.font}">${openNow ? 'OPEN' : 'CLOSED'}</div>`;
  })();

  const elements = (design.layout || [])
    .map((el, index) => {
      const box = `left:${el.x}px;top:${el.y}px;width:${el.w}px;height:${el.h}px;z-index:${index + 1}`;
      let inner = '';
      if (el.type === 'logo' && logoUrl) {
        ...
        inner = `<div class="title" style="font-size:${fs}px;font-family:${escapeHtml(ff)};font-weight:${layoutFontWeight(el, ff)};color:${fg}">${escapeHtml(
          customer.name || 'Hours'
        )}</div>`;
        ...
        const fs = resolveLayoutFontSize(el, design);
        const ff = resolveLayoutFontFamily(el, design);
        inner = `<div class="hours" style="font-size:${fs}px;font-family:${escapeHtml(ff)};font-weight:${layoutFontWeight(el, ff)};color:${fg}">${DAYS.map(
          (day) =>
        ...
        inner = `<div class="custom-text" style="justify-content:${justify};text-align:${align};font-size:${
          fs
        }px;font-family:${escapeHtml(ff)};color:${escapeHtml(color)};font-weight:${layoutFontWeight(el, ff)}">${escapeHtml(el.text || '')}</div>`;
      } else if (el.type === 'image' && el.mediaId) {
        const file = resolveImageFile(dataDir, customer, el.mediaId);
        ...
        const url = fileToDataUrl(file);
        if (url) {
          const color = el.color || (hexLooksDark(bg) ? '#FFFFFF' : '#000000');
          inner = symbolGlyphHtml(url, color);
        }
      }
      if (!inner) return '';
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .symbol-glyph {
    width: 100%; height: 100%; display: block; background-color: #000000;
