...
async function rotatePngBuffer(pngBuffer, degrees) {
...
html,body{margin:0;padding:0;width:${outW}px;height:${outH}px;overflow:hidden;background:#ffffff}
...
async function renderDisplayPng(customer, options = {}) {
  ...
  const eink = normalizeEink(customer.eink);
  const tablet = tabletFromEink(eink);
  const hangRotation = normalizeHangRotation(eink.hangRotation);
  const swapOut = hangRotation === 90 || hangRotation === 270;
  const outW = swapOut ? tablet.height : tablet.width;
  const outH = swapOut ? tablet.width : tablet.height;
  ...
  // If Chrome still emitted a swapped frame, correct it before hang rotation.
  const captured = readPngSize(buffer);
  const expectW = Math.round(tablet.width * scale);
  const expectH = Math.round(tablet.height * scale);
  if (captured && captured.width === expectH && captured.height === expectW) {
    buffer = await rotatePngBuffer(buffer, 90);
  }

  if (hangRotation) {
    buffer = await rotatePngBuffer(buffer, hangRotation);
  }
  ...
  return {
    buffer,
    version,
    width: (finalSize && finalSize.width) || outW,
    height: (finalSize && finalSize.height) || outH,
    contentType: 'image/png',
    cached: false
  };
}
...
function displayMeta(customer, baseUrl) {
  ...
  return {
    ...
    width: swapOut ? tablet.height : tablet.width,
    height: swapOut ? tablet.width : tablet.height,
    designWidth: tablet.width,
    designHeight: tablet.height,
