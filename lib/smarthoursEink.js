 ...
 * Side button (BTN_PIN, default GPIO0) wakes the board for a live poll.
...
// Side button: live refresh / wake from deep sleep (LOW = pressed)
#ifndef BTN_PIN
#define BTN_PIN 0
#endif
...
void postCheckin(uint32_t pollMinutes, uint8_t batteryPct, int batteryMv) {
  HTTPClient http;
  http.setTimeout(15000);
  if (!http.begin(checkinUrl())) return;
  http.addHeader("Content-Type", "application/json");
  StaticJsonDocument<512> doc;
  doc["deviceId"] = deviceIdHex();
  doc["customerSlug"] = SLUG;
...
void deepSleepMinutes(uint32_t minutes) {
  ...
  Serial.printf("Sleeping %u minutes (side button wakes for a live poll)...\\n", (unsigned)minutes);
...
void setup() {
  ...
  if (buttonWake) Serial.println("Side button wake — live poll");
  initDisplay();
  ...
  postCheckin(pollMinutesFallback, batteryPct, batteryMv);
