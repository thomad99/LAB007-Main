...
function setupPortalLogoDataUri() {
  return setupPortalAssetDataUri('smarthours-logo-white.png', 'image/png')
    || setupPortalAssetDataUri('smarthours-setup-logo.jpg', 'image/jpeg');
}

function setupPortalQrDataUri() {
  return setupPortalAssetDataUri('smarthours-setup-qr.png', 'image/png');
}
...
function buildEsp32Sketch(customer, options = {}) {
  ...
  const setupLogoImg = setupLogoSrc
    ...
    : '<h1>SmartHours</h1>';
  const setupQrSrc = setupPortalQrDataUri();
  const setupQrImg = setupQrSrc
    ? `<div class="qr-wrap"><img src="${setupQrSrc}" alt="Open lab007.ai/SmartHours"></div>`
    : '';
 ...
 * Join it, open the setup page, then: SCAN for WiFi → pick a network → password → customer name.
 *
 * Default poll: ${poll} minutes (server meta and check-in can override without reflash).
 * With Deep Sleep on (default), the server keeps that poll until 2 hours after closing,
 * then sleeps until 2 hours before the next opening. Buttons still wake for a live check-in.
...
<style>
body{font-family:system-ui,sans-serif;margin:0;background:#000;color:#fff;padding:24px;max-width:420px}
h1{font-size:1.3rem;margin:0 0 6px;text-align:center;color:#fff}
.brand{display:block;width:100%;max-width:320px;height:auto;margin:0 auto 16px;background:transparent;padding:0}
.account{color:#fff;margin:0 0 10px;line-height:1.45;text-align:center;font-size:.95rem}
.sub{color:#fff;margin:0 0 16px;line-height:1.4;text-align:center}
.qr-wrap{display:flex;justify-content:center;margin:0 0 20px}
.qr-wrap img{width:180px;height:180px;background:#fff;padding:10px;border-radius:10px;box-sizing:border-box}
...
<body>
${setupLogoImg}
<p class="account">Create your customer account online at www.lab007.ai/SmartHours</p>
${setupQrImg}
...
<form id="cfg" class="hidden" method="POST" action="/save">
  ...
  <div id="afterNet" class="hidden">
    ...
    <label for="slug">Customer Name (The name you created on Smarthours website)</label>
    <input id="slug" name="slug" autocomplete="off" placeholder="your shop name">
    ...
    for (var i = 0; i < list.length; i++) {
      (function(name) {
        ...
        b.addEventListener('click', function() {
          ...
          statusEl.textContent = 'Enter the WiFi password, then your customer name.';
...
void handleSetupSave() {
  ...
  if (!ssid.length() || !slug.length()) {
    setupServer.send(400, "text/plain", "WiFi network and customer name are required.");
    return;
  }
...
bool spiffsReady() {
  ...
  if (!tried) {
    ...
    if (!ok) Serial.println("SPIFFS mount failed — cannot cache OPEN/CLOSED images");
  }
  return ok;
}
...
void deepSleepMinutes(uint32_t minutes) {
  ...
  Serial.printf("Deep sleep %u minutes (short press = live check-in, hold 3s = setup)...\\n", (unsigned)minutes);
  Serial.flush();
  initButtons();
  waitButtonsReleased();
  delay(20);
