...
bool fetchMeta(String& versionOut, uint32_t& pollOut, String& imageOut) {
  ...
  imageOut = String((const char*)doc["bmpUrl"] | "");
  if (!imageOut.length()) imageOut = String((const char*)doc["imageUrl"] | "");
  if (imageOut.indexOf(".png") >= 0 || imageOut.indexOf("display.png") >= 0) {
...
void setup() {
  ...
  if (fetchMeta(version, pollMinutes, img)) {
    if (livePoll || version != last) {
      ...
      Serial.println("Design version unchanged — skipping panel refresh");
    }
    prefs.putUInt("poll", pollMinutes);
    pollMinutesFallback = pollMinutes;
  } else {
