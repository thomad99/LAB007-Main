...
function blitRgbaToGray(gray, rgbaImg, dx, dy, dw, dh, fit, tintLum) {
  ...
  const hasTint = Number.isFinite(tintLum);
  const paint = hasTint ? Math.max(0, Math.min(255, Math.round(tintLum))) : null;
  for (let y = 0; y < rh; y += 1) {
    ...
    for (let x = 0; x < rw; x += 1) {
      ...
      if (a < 0.08) continue;
      const srcLum = 0.299 * rgbaImg.rgba[i] + 0.587 * rgbaImg.rgba[i + 1] + 0.114 * rgbaImg.rgba[i + 2];
      const coverage = hasTint ? a * (srcLum / 255) : a;
      const lum = hasTint ? paint : Math.round(srcLum);
      const prev = gray.data[ty * gray.width + tx];
      setGray(gray, tx, ty, Math.round(prev * (1 - coverage) + lum * coverage));
    }
  }
}
...
function layoutPayload(customer) {
  ...
  return {
    ...
    design,
    eink,
    render: 'native-bmp-v2-symbol-tint'
  };
}
...
function layoutContentVersion(customer) {
  return hashContent(layoutPayload(customer));
}

function contentVersion(customer) {
  const hours = effectiveHours(customer);
  return hashContent({
    ...layoutPayload(customer),
    openNow: isOpenNow(hours, new Date(), customer.timeZone)
  });
}

function resolveRenderOpenNow(customer, forceOpenNow) {
  if (forceOpenNow === true || forceOpenNow === false) return forceOpenNow;
  return isOpenNow(effectiveHours(customer), new Date(), customer.timeZone);
}

function pruneEinkCache(dataDir, customerId, keepLayoutVersion) {
  const dir = einkCacheDir(dataDir);
  if (!fs.existsSync(dir)) return;
  const prefix = `${customerId}.`;
  const keep = new Set([
    `${customerId}.${keepLayoutVersion}.open.bmp`,
    `${customerId}.${keepLayoutVersion}.closed.bmp`
  ]);
...
function renderDesignNative(customer, dataDir, forceOpenNow) {
  const eink = normalizeEink(customer.eink);
  const tablet = tabletFromEink(eink);
  const design = normalizeDesign(customer.design, tablet, eink.colorMode);
  const hours = effectiveHours(customer);
  const openNow = resolveRenderOpenNow(customer, forceOpenNow);
  ...
  (design.layout || []).forEach((el) => {
    ...
    } else if (el.type === 'symbol' && el.symbolId) {
      const file = readSymbolFile(el.symbolId);
      const img = file ? decodeImageToRgba(file.buffer, file.type) : null;
      if (img) {
        const fallback = bg > 127 ? 0 : 255;
        const tint = parseHexColor(el.color || (bg > 127 ? '#000000' : '#FFFFFF'), fallback);
        blitRgbaToGray(gray, img, box.x, box.y, box.w, box.h, 'contain', tint);
      }
    }
  });
...
function renderLegacyHoursBmp(customer, forceOpenNow) {
  const eink = normalizeEink(customer.eink);
  const hours = effectiveHours(customer);
  const openNow = resolveRenderOpenNow(customer, forceOpenNow);
...
function renderEinkBmp(customer, options = {}) {
  ...
  ensureDirs(dataDir);
  const eink = normalizeEink(customer.eink);
  const layoutVersion = layoutContentVersion(customer);
  const openNow = resolveRenderOpenNow(customer, options.openNow);
  const statusKey = openNow ? 'open' : 'closed';
  const version =
    options.openNow === true || options.openNow === false
      ? `${layoutVersion}-${statusKey}`
      : contentVersion(customer);
  const cachePath = path.join(
    einkCacheDir(dataDir),
    `${customer.id}.${layoutVersion}.${statusKey}.bmp`
  );
  ...
  try {
    ...
    if (design.layout && design.layout.length) {
      buffer = renderDesignNative(customer, dataDir, openNow).buffer;
    } else {
      buffer = renderLegacyHoursBmp(customer, openNow);
    }
  } catch (err) {
  ...
  try {
    ...
    pruneEinkCache(dataDir, customer.id, layoutVersion);
  } catch (err) {
...
function einkMeta(customer, baseUrl) {
  ...
  const layoutVersion = layoutContentVersion(customer);
  const hours = effectiveHours(customer);
  const openNow = isOpenNow(hours, new Date(), customer.timeZone);
  const root = String(baseUrl || '').replace(/\/$/, '');
  const bmpBase = `${root}/api/smarthours/public/${encodeURIComponent(customer.slug)}/eink.bmp`;
  const setup = setupPortalMeta(root);
  return {
    slug: customer.slug,
    name: customer.name,
    version,
    layoutVersion,
    openNow,
    hours: compactDeviceHours(customer),
    updatedAt: customer.updatedAt,
    pollIntervalMinutes: eink.pollIntervalMinutes,
    width: eink.width,
    height: eink.height,
    inverted: eink.inverted,
    imageUrl: `${bmpBase}?v=${version}`,
    openBmpUrl: `${bmpBase}?v=${layoutVersion}&status=open`,
    closedBmpUrl: `${bmpBase}?v=${layoutVersion}&status=closed`,
    ...
    setupPageVersion: setup.setupPageVersion,
    setupPageUrl: setup.setupPageUrl
  };
}
...
function setupPortalAssetDataUri(fileName, mime) {
  const filePath = path.join(__dirname, '..', 'public', 'smarthours-assets', fileName);
  try {
...
function setupPortalLogoDataUri() {
  return setupPortalAssetDataUri('smarthours-logo-white.png', 'image/png');
}

let cachedSetupPortal = null;

function buildSetupPortalHtml() {
  if (cachedSetupPortal) return cachedSetupPortal.html;
  const setupLogoSrc = setupPortalLogoDataUri();
  const setupLogoImg = setupLogoSrc
    ? `<img class="brand" src="${setupLogoSrc}" alt="SmartHours">`
    : '<h1>SmartHours</h1>';
...
<style>
body{font-family:system-ui,sans-serif;margin:0;background:#000;color:#fff;padding:24px 24px 28px;max-width:420px}
h1{font-size:1.3rem;margin:0 0 6px;text-align:center;color:#fff}
.brand{display:block;width:100%;max-width:280px;height:auto;margin:0 auto 18px;background:transparent;padding:0}
.step{color:#fff;margin:0 0 16px;line-height:1.4;text-align:center;font-size:1.02rem}
button{width:100%;padding:16px;font-size:1.05rem;border:0;border-radius:12px;background:#38bdf8;color:#000;font-weight:700}
button.ghost{background:#1a1a1a;color:#fff;margin-top:10px;font-weight:600;text-align:left}
button.ghost.picked{outline:2px solid #38bdf8}
button.save{margin-top:18px}
label{display:block;margin:16px 0 6px;font-size:.85rem;color:#fff}
input{width:100%;box-sizing:border-box;padding:14px;border-radius:10px;border:1px solid #444;background:#111;color:#fff;font-size:1rem}
.hidden{display:none}
#status{min-height:1.4em;margin:12px 0;color:#fff}
#nets{margin-top:8px}
.footer{margin:28px 0 0;padding-top:16px;border-top:1px solid #333;color:#aaa;font-size:.82rem;line-height:1.45;text-align:center}
</style>
</head>
<body>
${setupLogoImg}
<p class="step">JOIN hotspot "MySmartLife"</p>
...
<form id="cfg" class="hidden" method="POST" action="/save">
  ...
  <div id="afterNet" class="hidden">
    ...
    <label for="slug">Unique customer ID</label>
    <input id="slug" name="slug" autocomplete="off">
    <button type="submit" class="save">CONNECT</button>
  </div>
</form>
<p class="footer">SHORT PRESS or DOUBLE CLICK Updates screen<br>LONG PRESS (3s) Resets</p>
<script>
...
function doScan() {
  ...
  fetch('/scan').then(function(r) { return r.json(); }).then(function(list) {
    ...
    for (var i = 0; i < list.length; i++) {
      (function(name) {
        ...
        b.addEventListener('click', function() {
          ...
          statusEl.textContent = 'Enter WIFI password, then Unique customer ID.';
    ...
    statusEl.textContent = 'Choose WIFI Name.';
    scanBtn.disabled = false;
  }).catch(function() {
...
scanBtn.addEventListener('click', doScan);
  ...
  cachedSetupPortal = {
    html,
    version: crypto.createHash('sha1').update(html).digest('hex').slice(0, 16)
  };
  return cachedSetupPortal.html;
}

function setupPortalVersion() {
  if (!cachedSetupPortal) buildSetupPortalHtml();
  return cachedSetupPortal.version;
}

function setupPortalMeta(baseUrl) {
  const root = String(baseUrl || '').replace(/\/$/, '');
  const version = setupPortalVersion();
  return {
    setupPageVersion: version,
    setupPageUrl: root ? `${root}/api/smarthours/public/esp32/setup.html?v=${encodeURIComponent(version)}` : ''
  };
}
 ...
 *
 * This generated sketch is NOT what production Spectra 6 units flash.
 * Flash firmware from: C:\Users\tomo\Documents\AI\22.SmartHours-ESP32\SmartHours_ESP32
 *
 ...
 *  - Then it polls the server; when the design/hours layout changes, it downloads
 *    OPEN and CLOSED BMPs. Between polls it flips OPEN/CLOSED from stored hours
 *    even if Wi-Fi is down. The web check is only to see if hours or the design changed.
 *
 * Setup
 * 1) Arduino IDE → ESP32 board support
 * 2) Libraries: WiFi, HTTPClient, ArduinoJson, WebServer, DNSServer, GxEPD2, Adafruit GFX
 * 3) Default panel is 7.5" 800x480 (GxEPD2_750_T7). Waveshare ESP32 driver board:
 *    add #define SMARTHOURS_WS_ESP32_DRIVER 1 before the includes.
 * 4) Set BASE_URL, flash. Leave WIFI / customer name empty to use the setup screen.
 *
 * First boot / after reset: the board broadcasts WiFi "MySmartLife".
 * Join it, open the setup page, then: SCAN for WiFi → pick a network → password → Unique customer ID → CONNECT.
 * A fallback setup page is baked in. After the board is online it downloads the latest
 * setup HTML from the server and caches it, so portal copy/logo updates do not need a reflash.
 *
 * Default poll: ${poll} minutes (server meta and check-in can override without reflash).
 * With Deep Sleep on (default), the server keeps that poll until the configured hours after closing,
 * then sleeps until the configured hours before the next opening. Buttons still wake for a live check-in.
 * Between polls the ESP32 deep-sleeps, Wi-Fi is powered off, and the e-paper is hibernated
 * (Waveshare: sleep or power off the panel after every refresh).
 * Waveshare ESP32 Driver Board: set switch 2 (USB-UART) OFF on battery to save ~1 mA.
 * Any button: short press or double-click = live check-in / screen refresh;
 * hold 3 seconds = reset to setup (MySmartLife).
 * USB Serial Monitor (115200, newline): after a USB reset type help (skipped on timer/button wake).
 * Battery ADC is reported on check-in. Offline WiFi keeps last hours and swaps OPEN/CLOSED locally.
 */
...
// E-ink: install GxEPD2 + Adafruit GFX. Default panel is Waveshare 7.5" V2 800x480 (GDEW075T7).
// Waveshare ESP32 Driver Board: set SMARTHOURS_WS_ESP32_DRIVER 1 below (HSPI pins).
#ifndef SMARTHOURS_USE_GXEPD2
#define SMARTHOURS_USE_GXEPD2 1
#endif
#ifndef SMARTHOURS_WS_ESP32_DRIVER
#define SMARTHOURS_WS_ESP32_DRIVER 0
#endif
#if SMARTHOURS_USE_GXEPD2
#include <GxEPD2_BW.h>
#endif
...
const char* FW_VERSION = "1.9.2";
const char* SETUP_AP_NAME = "MySmartLife";
const char* SETUP_HTML_PATH = "/setup.htm";
...
// All four keys share one handler. Short press or double-click = refresh.
// Hold BTN_LONG_MS (3s) = reset to setup. Waveshare ESP32 Driver Board:
//   BOOT = GPIO0, KEY1 = GPIO39, KEY2 = GPIO34, KEY3 = GPIO35
...
#endif
#ifndef BTN_DOUBLE_MS
#define BTN_DOUBLE_MS 450
#endif
const uint8_t BTN_PINS[] = { BTN_PIN_LIST };
const size_t BTN_COUNT = sizeof(BTN_PINS) / sizeof(BTN_PINS[0]);
enum { BTN_NONE = 0, BTN_SHORT = 1, BTN_LONG = 2, BTN_DOUBLE = 3 };
// Battery ADC on a pin that is NOT a key. GPIO34 is KEY2 on the Waveshare board.
#ifndef BAT_ADC_PIN
...
// SPI pins — ESP32 DevKit + HAT on VSPI (CS/DC/RST/BUSY). SCK=18 MOSI=23.
// Waveshare ESP32 e-Paper Driver Board: #define SMARTHOURS_WS_ESP32_DRIVER 1
#if SMARTHOURS_WS_ESP32_DRIVER
#ifndef EPD_CS
#define EPD_CS   15
#define EPD_DC   27
#define EPD_RST  26
...
void clearSavedWifi() {
  ...
  prefs.putString("ver", "");
  prefs.putString("lay", "");
  prefs.putString("hours", "");
  prefs.putUChar("shown", 0);
  cfgSsid = "";
  cfgPass = "";
  cfgSlug = "";
}

bool pinIsButton(uint8_t pin) {
...
bool waitForButtonState(bool down, uint32_t timeoutMs) {
  uint32_t t0 = millis();
  while ((millis() - t0) < timeoutMs) {
    if (anyButtonDown() == down) {
      delay(BTN_DEBOUNCE_MS);
      if (anyButtonDown() == down) return true;
    }
    delay(10);
  }
  return anyButtonDown() == down;
}

int classifyHeldPress() {
  uint32_t t0 = millis();
  while (anyButtonDown()) {
...
int classifyButtonPress() {
  bool fromSleep = esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT1;
  int first = BTN_NONE;
  if (anyButtonDown()) {
    delay(BTN_DEBOUNCE_MS);
    if (anyButtonDown()) first = classifyHeldPress();
  } else if (fromSleep) {
    first = BTN_SHORT;
  } else {
    return BTN_NONE;
  }
  if (first == BTN_LONG) return BTN_LONG;
  if (waitForButtonState(true, BTN_DOUBLE_MS)) {
    int second = classifyHeldPress();
    if (second == BTN_LONG) return BTN_LONG;
    return BTN_DOUBLE;
  }
  return BTN_SHORT;
}
...
void armButtonWake() {
  ...
  for (size_t i = 0; i < BTN_COUNT; i++) {
    ...
    mask |= (1ULL << p);
    if (rtc_gpio_is_valid_gpio((gpio_num_t)p) && p < 34) {
...
void handleSetupRoot() {
  if (spiffsReady() && SPIFFS.exists(SETUP_HTML_PATH)) {
    File f = SPIFFS.open(SETUP_HTML_PATH, FILE_READ);
    if (f && f.size() > 200) {
      Serial.printf("Serving cached setup page (%u bytes)\\n", (unsigned)f.size());
      setupServer.streamFile(f, "text/html");
      f.close();
      return;
    }
    if (f) f.close();
...
void handleSetupSave() {
  ...
  if (!ssid.length() || !slug.length()) {
    setupServer.send(400, "text/plain", "WIFI network and Unique customer ID are required.");
    return;
  }
  ...
  setupServer.send(200, "text/html",
    "<html><body style='font-family:sans-serif;background:#000;color:#fff;padding:24px'>"
...
uint8_t readBatteryPercent(int& mvOut) {
  mvOut = 0;
  if (pinIsButton(BAT_ADC_PIN)) return 0;
...
#if SMARTHOURS_USE_GXEPD2
// Page buffer HEIGHT/4 (~12KB) leaves RAM for WiFi + the downloaded BMP.
GxEPD2_BW<GxEPD2_750_T7, GxEPD2_750_T7::HEIGHT / 4> display(GxEPD2_750_T7(EPD_CS, EPD_DC, EPD_RST, EPD_BUSY));
#if SMARTHOURS_WS_ESP32_DRIVER
SPIClass epdSpi(HSPI);
#endif
#endif
bool panelInited = false;

void holdGpioLow(uint8_t pin) {
  pinMode(pin, OUTPUT);
...
void releaseSleepPinHolds() {
  ...
  gpio_hold_dis((gpio_num_t)EPD_SCK);
  gpio_hold_dis((gpio_num_t)EPD_MOSI);
}

bool initDisplay() {
  if (panelInited) return true;
#if SMARTHOURS_USE_GXEPD2
  releaseSleepPinHolds();
#if SMARTHOURS_WS_ESP32_DRIVER
  epdSpi.begin(EPD_SCK, 12, EPD_MOSI, EPD_CS);
  display.epd2.selectSPI(epdSpi, SPISettings(4000000, MSBFIRST, SPI_MODE0));
#endif
  // Waveshare: keep RST pulse short so the 5V "clever reset" circuit does not cut panel power mid-init.
  display.init(0, true, 2, false);
  display.setRotation(0);
  panelInited = true;
  Serial.printf("Display ready %ux%u  CS=%u DC=%u RST=%u BUSY=%u\\n",
    EINK_W, EINK_H, EPD_CS, EPD_DC, EPD_RST, EPD_BUSY);
  return true;
#else
  Serial.printf("Display target %ux%u (GxEPD2 disabled)\\n", EINK_W, EINK_H);
  return false;
#endif
}
...
void hibernatePanel() {
  ...
  // Hold SPI/RST low in deep sleep so the panel stays in sleep and GPIOs do not leak
  // through the Waveshare level shifter (GxEPD2 / Waveshare low-power notes).
  holdGpioLow(EPD_RST);
  holdGpioLow(EPD_CS);
  holdGpioLow(EPD_DC);
  holdGpioLow(EPD_SCK);
  holdGpioLow(EPD_MOSI);
...
void drawPackedToPanel(const uint8_t* packed, uint16_t w, uint16_t h) {
  Serial.printf("Painting packed 1-bit bitmap %ux%u (%u bytes)\\n",
  ...
  do {
    ...
    // SmartHours packed BMP uses 0 = black, Adafruit drawBitmap uses 1 = color.
    display.drawInvertedBitmap(0, 0, packed, w, h, GxEPD_BLACK);
  } while (display.nextPage());
  Serial.println("Panel refresh done");
#else
  (void)packed; (void)w; (void)h;
  Serial.println("GxEPD2 disabled — panel not updated");
#endif
}
...
const char* OPEN_BMP_PATH = "/open.bmp";
const char* CLOSED_BMP_PATH = "/closed.bmp";
...
bool spiffsReady() {
  ...
  if (!tried) {
    ...
    if (!ok) Serial.println("SPIFFS mount failed — cannot cache images or setup page");
  }
  return ok;
}
...
void applyPosixTz(const char* posix) {
  if (!posix || !posix[0]) posix = "EST5EDT,M3.2.0,M11.1.0";
  setenv("TZ", posix, 1);
  tzset();
}

bool syncNtp() {
  String hoursJson = prefs.getString("hours", "");
  if (hoursJson.length()) {
    StaticJsonDocument<1536> doc;
    if (!deserializeJson(doc, hoursJson)) applyPosixTz(doc["tz"] | "EST5EDT,M3.2.0,M11.1.0");
  } else {
    applyPosixTz("EST5EDT,M3.2.0,M11.1.0");
  }
...
int hmToMins(const char* hm) {
  if (!hm || strlen(hm) < 4) return -1;
  int h = atoi(hm);
...
bool slotContains(int mins, const char* openHm, const char* closeHm) {
  int start = hmToMins(openHm);
  int end = hmToMins(closeHm);
...
bool localIsOpen() {
  ...
  String hoursJson = prefs.getString("hours", "");
  if (!hoursJson.length()) return false;
  StaticJsonDocument<1536> doc;
  if (deserializeJson(doc, hoursJson)) return false;
  applyPosixTz(doc["tz"] | "EST5EDT,M3.2.0,M11.1.0");
  time_t now = time(nullptr);
  struct tm t;
  localtime_r(&now, &t);
  const char* keys[] = {"sun", "mon", "tue", "wed", "thu", "fri", "sat"};
  JsonArray slots = doc["days"][keys[t.tm_wday]];
  if (slots.isNull() || !slots.size()) return false;
  int mins = t.tm_hour * 60 + t.tm_min;
  for (JsonVariant slot : slots) {
    if (!slot.is<JsonArray>() || slot.size() < 2) continue;
    if (slotContains(mins, slot[0] | "", slot[1] | "")) return true;
  }
  return false;
}

time_t localHmEpoch(struct tm base, int dayOffset, int mins) {
  struct tm t = base;
  t.tm_mday += dayOffset;
  t.tm_hour = mins / 60;
  t.tm_min = mins % 60;
...
uint32_t minutesUntilLocalStatusChange() {
  if (!timeIsValid()) return 0;
  String hoursJson = prefs.getString("hours", "");
  if (!hoursJson.length()) return 0;
  StaticJsonDocument<1536> doc;
  if (deserializeJson(doc, hoursJson)) return 0;
  applyPosixTz(doc["tz"] | "EST5EDT,M3.2.0,M11.1.0");
  time_t now = time(nullptr);
  struct tm base;
  ...
  for (int dayOffset = 0; dayOffset < 8; dayOffset++) {
    int wday = (base.tm_wday + dayOffset) % 7;
    JsonArray slots = doc["days"][keys[wday]];
    if (slots.isNull()) continue;
    for (JsonVariant slot : slots) {
      if (!slot.is<JsonArray>() || slot.size() < 2) continue;
      int start = hmToMins(slot[0] | "");
      int end = hmToMins(slot[1] | "");
      if (start < 0 || end < 0) continue;
      time_t openAt = localHmEpoch(base, dayOffset, start);
      time_t closeAt = (end <= start)
        ? localHmEpoch(base, dayOffset + 1, end)
        : localHmEpoch(base, dayOffset, end);
      if (openAt > now && (!best || openAt < best)) best = openAt;
      if (closeAt > now && (!best || closeAt < best)) best = closeAt;
    }
  }
  if (!best) return 0;
  int mins = (int)((best - now + 59) / 60);
  if (mins > 2) mins -= 2;
  if (mins < 1) mins = 1;
  return (uint32_t)mins;
}

uint32_t sleepMinutesWithLocalCutover(uint32_t poll) {
  if (poll < 1) poll = 1;
  uint32_t until = minutesUntilLocalStatusChange();
  if (until >= 1 && until < poll) return until;
  return poll;
}

bool haveStatusFiles() {
  return spiffsReady() && SPIFFS.exists(OPEN_BMP_PATH) && SPIFFS.exists(CLOSED_BMP_PATH);
}

bool fetchToFile(const String& url, const char* path) {
  if (!url.length() || !spiffsReady()) return false;
  HTTPClient http;
  http.setTimeout(45000);
  if (!http.begin(url)) return false;
  int code = http.GET();
  if (code != 200) {
    Serial.printf("Image HTTP %d for %s\\n", code, path);
    http.end();
    return false;
  }
  int len = http.getSize();
  const int maxLen = 250000;
  if (len > maxLen) { http.end(); return false; }
  File f = SPIFFS.open(path, FILE_WRITE);
  if (!f) { http.end(); return false; }
  WiFiClient* stream = http.getStreamPtr();
  uint8_t buf[1024];
  int rd = 0;
  int cap = (len > 0) ? len : maxLen;
  unsigned long startMs = millis();
  while (rd < cap && (millis() - startMs) < 45000UL) {
...
bool drawFile(const char* path, const char* tag) {
  if (!spiffsReady() || !SPIFFS.exists(path)) return false;
  File f = SPIFFS.open(path, FILE_READ);
  if (!f) return false;
  size_t len = f.size();
  if (len < 62 || len > 250000) { f.close(); return false; }
  uint8_t* buf = (uint8_t*)malloc(len);
  if (!buf) { f.close(); return false; }
  size_t got = f.read(buf, len);
  f.close();
  bool ok = got >= 62 && drawBmpBuffer(buf, got, tag);
  free(buf);
  return ok;
}

bool paintLocalStatus(bool wantOpen, bool force) {
  uint8_t want = wantOpen ? 1 : 2;
  uint8_t shown = prefs.getUChar("shown", 0);
  if (!force && shown == want) {
    Serial.println(wantOpen ? "Panel already OPEN" : "Panel already CLOSED");
    return true;
  }
  const char* path = wantOpen ? OPEN_BMP_PATH : CLOSED_BMP_PATH;
  const char* tag = wantOpen ? "cached OPEN" : "cached CLOSED";
  if (!drawFile(path, tag)) return false;
  prefs.putUChar("shown", want);
  return true;
}
...
struct EinkMeta {
  ...
  uint32_t pollMinutes;
  String imageUrl;
  String openBmpUrl;
  String closedBmpUrl;
  String hoursJson;
  String setupPageVersion;
  String setupPageUrl;
  bool openNow;
  bool ok;
};

bool fetchMetaFull(EinkMeta& meta) {
  meta.ok = false;
  meta.pollMinutes = pollMinutesFallback;
  meta.openNow = false;
  HTTPClient http;
  ...
  meta.version = String((const char*)doc["version"] | "");
  meta.layoutVersion = String((const char*)doc["layoutVersion"] | "");
  if (!meta.layoutVersion.length()) meta.layoutVersion = meta.version;
  meta.pollMinutes = doc["pollIntervalMinutes"] | pollMinutesFallback;
  meta.openNow = doc["openNow"] | false;
  meta.imageUrl = String((const char*)doc["bmpUrl"] | "");
  if (!meta.imageUrl.length()) meta.imageUrl = String((const char*)doc["imageUrl"] | "");
  if (meta.imageUrl.indexOf(".png") >= 0 || meta.imageUrl.indexOf("display.png") >= 0) {
    Serial.println("Server offered PNG — using eink.bmp for the panel");
    meta.imageUrl = "";
  }
  if (!meta.imageUrl.length() && meta.version.length()) meta.imageUrl = imageUrl(meta.version);
  meta.openBmpUrl = String((const char*)doc["openBmpUrl"] | "");
  meta.closedBmpUrl = String((const char*)doc["closedBmpUrl"] | "");
  meta.setupPageVersion = String((const char*)doc["setupPageVersion"] | "");
  meta.setupPageUrl = String((const char*)doc["setupPageUrl"] | "");
  if (doc.containsKey("hours") && doc["hours"].is<JsonObject>()) {
    String hours;
    serializeJson(doc["hours"], hours);
    meta.hoursJson = hours;
  }
  meta.ok = meta.version.length() > 0 || meta.layoutVersion.length() > 0;
  return meta.ok;
}

bool fetchMeta(String& versionOut, uint32_t& pollOut, String& imageOut) {
  EinkMeta meta;
  if (!fetchMetaFull(meta)) return false;
  versionOut = meta.version;
  pollOut = meta.pollMinutes;
  imageOut = meta.imageUrl;
  return true;
}

bool refreshStatusImages(const EinkMeta& meta) {
  bool okOpen = meta.openBmpUrl.length() && fetchToFile(meta.openBmpUrl, OPEN_BMP_PATH);
  bool okClosed = meta.closedBmpUrl.length() && fetchToFile(meta.closedBmpUrl, CLOSED_BMP_PATH);
  if (!okOpen && !okClosed && meta.imageUrl.length()) {
    const char* path = meta.openNow ? OPEN_BMP_PATH : CLOSED_BMP_PATH;
    return fetchToFile(meta.imageUrl, path);
  }
  return okOpen || okClosed;
}

bool syncSetupPage(const EinkMeta& meta) {
  if (!meta.setupPageVersion.length() || !meta.setupPageUrl.length()) return false;
  String have = prefs.getString("spver", "");
  if (have == meta.setupPageVersion && spiffsReady() && SPIFFS.exists(SETUP_HTML_PATH)) {
    return true;
  }
  Serial.printf("Setup page %s -> %s\\n", have.c_str(), meta.setupPageVersion.c_str());
  if (!fetchToFile(meta.setupPageUrl, SETUP_HTML_PATH)) {
    Serial.println("Setup page download failed");
    return false;
  }
  prefs.putString("spver", meta.setupPageVersion);
  return true;
}
...
bool postCheckin(uint32_t pollMinutes, uint8_t batteryPct, int batteryMv) {
  ...
  if (code == 200) {
    ...
    if (!deserializeJson(out, resp)) {
      ...
      if (serverPoll >= 1) {
        if (serverPoll > 10080) serverPoll = 10080;
        pollMinutesFallback = serverPoll;
        prefs.putUInt("poll", serverPoll);
        const char* sleepMode = out["sleepMode"] | "poll";
        Serial.printf("Next sleep: %u min (%s)\\n", (unsigned)serverPoll, sleepMode);
      }
      if (out["reset"] | false) wantSetup = true;
      const char* cmd = out["command"] | "";
      if (cmd && strcmp(cmd, "setup") == 0) wantSetup = true;
      EinkMeta setupMeta;
      setupMeta.setupPageVersion = String((const char*)out["setupPageVersion"] | "");
      setupMeta.setupPageUrl = String((const char*)out["setupPageUrl"] | "");
      if (setupMeta.setupPageVersion.length() && setupMeta.setupPageUrl.length()) {
        syncSetupPage(setupMeta);
      }
    }
  } else {
...
bool fetchAndDrawImage(const String& url) {
  ...
  int len = http.getSize();
  WiFiClient* stream = http.getStreamPtr();
  // 800x480 1-bit BMP ~48KB; allow up to 250KB. Chunked replies report len=-1.
  const int maxLen = 250000;
  if (len > maxLen) { http.end(); return false; }
  int cap = (len > 0) ? len : maxLen;
  uint8_t* buf = (uint8_t*)malloc(cap);
  if (!buf) { http.end(); Serial.println("Out of memory for image download"); return false; }
  int rd = 0;
  if (len > 0) {
    rd = stream->readBytes(buf, len);
  } else {
    while (http.connected() && rd < cap) {
      int avail = stream->available();
      if (avail <= 0) {
        delay(10);
        if (!stream->available()) break;
      }
      int n = stream->readBytes(buf + rd, cap - rd);
      if (n <= 0) break;
      rd += n;
    }
  }
  http.end();
  bool ok = false;
  if (rd >= 62) ok = drawBmpBuffer(buf, rd, "server canvas");
  else Serial.printf("Image too small (%d bytes)\\n", rd);
  free(buf);
  return ok;
}
...
void deepSleepMinutes(uint32_t minutes) {
  ...
  Serial.printf("Deep sleep %u minutes (short/double-click = refresh, hold 3s = setup)...\\n", (unsigned)minutes);
  Serial.flush();
  initButtons();
  waitButtonsReleased();
  delay(20);
...
void maybeUsbConsole() {
  esp_sleep_wakeup_cause_t cause = esp_sleep_get_wakeup_cause();
  if (cause == ESP_SLEEP_WAKEUP_TIMER || cause == ESP_SLEEP_WAKEUP_EXT1) return;
  serialTestLoop();
}
...
void serialTestLoop() {
  Serial.println();
  Serial.println(F("USB test console @ 115200. Type help then Enter. Sleeps in 8s if idle."));
  uint32_t last = millis();
  uint32_t idleMs = 8000UL;
  String acc;
  while (true) {
...
void setup() {
  ...
  spiffsReady();
  initButtons();
  Serial.printf("Buttons: BOOT/KEY1/KEY2/KEY3 — short or double-click = refresh, hold 3s = setup\\n");
  ...
  bool livePoll = (btn == BTN_SHORT || btn == BTN_DOUBLE);
  if (btn == BTN_DOUBLE) Serial.println("Double click — live check-in");
  else if (livePoll) Serial.println("Short press — live check-in");

  if (!isConfigured()) {
    runSetupPortal();
  }

  String lastLayout = prefs.getString("lay", "");
  String last = prefs.getString("ver", "");
  bool painted = false;

  auto paintFromSchedule = [&](bool force) -> bool {
    if (!haveStatusFiles()) return false;
    bool wantOpen;
    if (timeIsValid() && prefs.getString("hours", "").length()) {
      wantOpen = localIsOpen();
      Serial.println(wantOpen ? "Local hours: OPEN" : "Local hours: CLOSED");
    } else if (!force) {
      Serial.println("Clock/hours unavailable — leaving panel as-is");
      return prefs.getUChar("shown", 0) != 0;
    } else {
      wantOpen = prefs.getUChar("shown", 1) != 2;
    }
    return paintLocalStatus(wantOpen, force);
  };

  if (!connectWifi()) {
    Serial.println("WiFi failed — using stored hours for OPEN/CLOSED");
    painted = paintFromSchedule(livePoll);
    ...
    goToSleep(sleepMinutesWithLocalCutover(pollMinutesFallback));
    return;
  }
  ...
  if (syncNtp()) Serial.println("NTP time synced");
  else Serial.println("NTP failed — using RTC if valid");

  EinkMeta meta;
  if (fetchMetaFull(meta)) {
    if (meta.hoursJson.length()) {
      prefs.putString("hours", meta.hoursJson);
      StaticJsonDocument<1536> hoursDoc;
      if (!deserializeJson(hoursDoc, meta.hoursJson)) {
        applyPosixTz(hoursDoc["tz"] | "EST5EDT,M3.2.0,M11.1.0");
      }
    }
    bool layoutChanged = meta.layoutVersion.length() && meta.layoutVersion != lastLayout;
    bool needFiles = !haveStatusFiles();
    if (layoutChanged || needFiles) {
      Serial.printf("Layout update %s -> %s\\n", lastLayout.c_str(), meta.layoutVersion.c_str());
      if (refreshStatusImages(meta)) {
        prefs.putString("lay", meta.layoutVersion);
        prefs.putString("ver", meta.version);
        prefs.putUChar("shown", 0);
      } else {
        Serial.println("OPEN/CLOSED image download failed");
      }
    } else {
      Serial.println("Hours/design unchanged — using cached OPEN/CLOSED images");
    }
    painted = paintFromSchedule(livePoll);
    if (!painted && meta.imageUrl.length()) {
      painted = fetchAndDrawImage(meta.imageUrl);
      if (painted) prefs.putString("ver", meta.version);
    }
    if (meta.pollMinutes >= 1) {
      pollMinutesFallback = meta.pollMinutes;
      prefs.putUInt("poll", meta.pollMinutes);
    }
    syncSetupPage(meta);
  } else {
    Serial.println("Meta fetch failed — using stored hours");
    painted = paintFromSchedule(livePoll);
  }
