...
function registerVideoCreatorRoutes(app) {
  ...
  app.get('/api/videocreator/youtube-discovery-status', (req, res) => {
    ...
    return res.json({
      ...
      hint: 'Set YOUTUBE_DATA_API_KEY (or YOUTUBE_API_KEY). Optional: YOUTUBE_DISCOVERY_REGION (default US).'
    });
  });
  ...
  app.get('/api/videocreator/youtube-discovery', async (req, res) => {
    try {
      ...
      return res.status(status).json({ ok: false, error: e.message || 'YouTube discovery failed.' });
    }
  });
