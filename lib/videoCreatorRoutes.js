...
} = require('./videoCreatorYoutubeDiscovery');
...
function registerVideoCreatorRoutes(app) {
  ...
  /** YouTube Data API v3 — trending ideas in a category + time window (requires API key). */
  app.get('/api/videocreator/youtube-discovery-status', (req, res) => {
    const key = getYoutubeDataApiKey();
    return res.json({
      ok: true,
      configured: Boolean(key),
      hint:
        'Set YOUTUBE_DATA_API_KEY (or YOUTUBE_API_KEY). Server-side calls need an API key not restricted to HTTP referrers only — use IP restriction or None for dev. Optional: YOUTUBE_DISCOVERY_REGION (default US).'
    });
  });

  app.get('/api/videocreator/youtube-discovery', async (req, res) => {
    try {
      ...
      const range = req.query.range ? String(req.query.range).toLowerCase() : '1m';
      const mx = req.query.maxVideos != null ? parseInt(String(req.query.maxVideos), 10) : NaN;
      const maxVideos = Number.isFinite(mx) ? mx : undefined;
      const searchQuery =
        req.query.q !== undefined && req.query.q !== null ? String(req.query.q) : '';
      ...
      const result = await discoverYoutubeVideos(wrappedFetch, {
        categoryId,
        range,
        maxVideos,
        searchQuery
      ...
      const msg = e.message || 'YouTube discovery failed.';
      const hint = hintForYoutubeDiscoveryError(msg);
