...
function registerSmartHoursRoutes(app) {
  ...
  app.post('/api/smarthours/devices/checkin', (req, res) => {
    try {
      ...
      const assigned = String(device.customerSlug || '');
      const reported = String(device.reportedSlug || '');
      res.json({
        ok: true,
        deviceId: device.id,
        customerId: assigned,
        changed: Boolean(assigned && reported && assigned !== reported),
  ...
  app.get('/api/smarthours/devices', (req, res) => {
    try {
  ...
  app.put('/api/smarthours/devices/:id', (req, res) => {
    try {
