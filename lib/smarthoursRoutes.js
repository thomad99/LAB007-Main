...
function registerSmartHoursRoutes(app) {
  ...
  app.get('/SmartHours/e/:slug', (req, res) => sendSmartHoursApp(res));
  app.get('/smarthours/e/:slug', (req, res) => {
    noIndex(res);
    res.redirect(301, `/SmartHours/e/${encodeURIComponent(req.params.slug)}`);
  });
  ...
  app.get('/api/smarthours/studio/:slug', (req, res) => {
    try {
      ...
      const devices = (store.devices || []).filter((d) => d.customerSlug === customer.slug);
      res.json({
        customer: {
          ...customer,
  ...
  app.put('/api/smarthours/customers/:id', async (req, res) => {
    try {
      ...
      if (body.slug != null || body.name != null) {
        const wanted = smarthours.requestedSlug(body.slug || prev.slug || body.name || prev.name);
        const check = smarthours.checkSlugAvailable(wanted, store.customers, prev.id);
        if (!check.available) {
          return res.status(409).json({ error: check.error, slug: check.slug });
        }
  ...
  app.post('/api/smarthours/devices/checkin', (req, res) => {
    try {
      ...
      const assigned = reset ? '' : String(device.customerSlug || '');
      ...
      res.json({
        ...
        reset,
        command: reset ? 'setup' : '',
        changed: Boolean(!reset && assigned && reported && assigned !== reported),
