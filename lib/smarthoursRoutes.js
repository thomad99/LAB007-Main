...
function registerSmartHoursRoutes(app) {
  ...
  async function sendStudioAccessEmail(req, customer, token, expiresAt) {
    ...
    await mailer.sendMail({
      from,
      to: customer.email,
      subject: 'Your SmartHours portal link',
      text:
        `Hi ${customer.name || 'there'},\n\n` +
        `Use this secured link to manage your SmartHours display. It stays valid for 10 days:\n\n` +
        `${link}\n\n` +
        `If you open SmartHours from another device, enter this email address on the landing page and we will send a fresh link.\n\n` +
        `Link expires: ${exp.toUTCString()}\n`,
      html:
        `<p>Hi ${String(customer.name || 'there').replace(/</g, '')},</p>` +
        `<p>Use this secured link to manage your SmartHours display. It stays valid for <strong>10 days</strong>.</p>` +
        `<p><a href="${link}">${link}</a></p>` +
        `<p>If you open SmartHours from another device, enter this email address on the landing page and we will send a fresh link.</p>` +
        `<p>Link expires: ${exp.toUTCString()}</p>`
    });
    return link;
  }
  ...
  app.post('/api/smarthours/auth/request-link', async (req, res) => {
    try {
      ...
      res.json({
        ...
        message:
          sent.length > 1
            ? 'We emailed a secured link for each of your SmartHours pages. The link is valid for 10 days.'
            : 'We emailed a secured link to your portal. The link is valid for 10 days.'
      });
    } catch (err) {
