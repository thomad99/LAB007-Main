...
function registerSmartHoursRoutes(app) {
  ...
  async function sendStudioAccessEmail(req, customer, token, expiresAt) {
    ...
    const logoPath = path.join(assetsDir, 'smarthours-logo-white.png');
    const logoUrl = fs.existsSync(logoPath)
      ? 'cid:smarthours-lockup'
      : `${base}/SmartHours/assets/smarthours-logo-white.png`;
    ...
    if (logoUrl.indexOf('cid:') === 0) {
      mail.attachments = [
        {
          filename: 'smarthours-logo-white.png',
          path: logoPath,
          cid: 'smarthours-lockup'
        }
      ];
  ...
  function lowBatteryEmailContent(alert, logoUrl) {
    const name = String(alert.customerName || 'Customer').trim() || 'Customer';
    const deviceName = String(alert.deviceName || 'your display').trim() || 'your display';
    const safeName = escapeEmailHtml(name);
    const safeDevice = escapeEmailHtml(deviceName);
    const safeLogo = escapeEmailHtml(logoUrl);
    return {
      subject: 'SmartHour Device - Battery Warning',
      text:
        `Dear ${name}\n\n` +
        `A friendly warning the battery on Device ${deviceName} has less than 10% charge.\n` +
        `Please consider to charge it.\n\n` +
        `Regards\n` +
        `MySmartHours Battery Health Bot\n`,
      html:
        `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#0b0f18;">` +
        `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0b0f18;">` +
        `<tr><td align="center" style="padding:24px 12px;">` +
        ...
        `<img src="${safeLogo}" alt="SmartHours" width="280" style="display:block;width:280px;max-width:100%;height:auto;border:0;">` +
        `</td></tr>` +
        `<tr><td style="padding:28px 24px 24px;font-family:Arial,Helvetica,sans-serif;color:#f4f7fb;">` +
        `<p style="margin:0 0 16px;font-size:16px;line-height:1.55;">Dear ${safeName}</p>` +
        `<p style="margin:0 0 16px;font-size:16px;line-height:1.55;">A friendly warning the battery on Device ${safeDevice} has less than 10% charge.</p>` +
        `<p style="margin:0 0 24px;font-size:16px;line-height:1.55;">Please consider to charge it.</p>` +
        `<p style="margin:0;font-size:16px;line-height:1.55;">Regards<br>MySmartHours Battery Health Bot</p>` +
        `</td></tr>` +
        `</table></td></tr></table></body></html>`
    };
  }

  async function sendLowBatteryAlertEmail(req, alert) {
    const mailer = studioMailer();
    if (!mailer) {
    ...
    const from = process.env.SMTP_FROM
      ? `${process.env.SMTP_FROM} <${process.env.SMTP_USER}>`
      : process.env.SMTP_USER;
    const content = lowBatteryEmailContent(alert, logoUrl);
    const mail = {
      from,
      to: alert.email,
      subject: content.subject,
  ...
  function notifyLowBatteryAlert(req, device) {
    const alert = device && device.pendingLowBatteryAlert;
    if (!alert || !alert.email) return;
    sendLowBatteryAlertEmail(req, alert).catch((err) => {
      smarthours.clearLowBatteryAlerted(dataDir, device.id);
      console.error('[SmartHours] low battery email failed:', err.message);
    });
  }
  ...
  function studioMeta(store) {
    ...
    return {
      ...
      symbols: smarthours.listSymbolLibrary()
    };
  }
    ...
    try {
      ...
      const item = smarthours.saveTemplateFile(
        ...
        req.body && req.body.name,
        {
          ...
          orientation: req.body && req.body.orientation,
  ...
  app.get('/api/smarthours/studio/:slug', async (req, res) => {
    try {
      ...
      const devices = smarthours
        ...
        .map((d) => (isAdminAuthed(req) ? d : smarthours.customerFacingDevice(d)));
      res.json({
        customer: {
  ...
  app.get('/SmartHours/ad', (req, res) => {
    res.setHeader('X-Robots-Tag', 'noindex, nofollow');
    res.sendFile(path.join(assetsDir, 'about', 'advert.html'));
  });
  app.get('/smarthours/ad', (req, res) => {
    res.redirect(301, '/SmartHours/ad');
  });
  ...
  app.get('/SmartHours/:slug', (req, res, next) => {
    ...
    if (/^(about|assets|e|v|ad)$/i.test(slug)) return next();
  ...
  app.get('/api/smarthours/customers', requireAdmin, (req, res) => {
    try {
      ...
      res.json({
        ...
        settings: smarthours.normalizeSettings(store.settings),
        ...studioMeta(store)
      });
    } catch (err) {
  ...
  app.put('/api/smarthours/settings', requireAdmin, (req, res) => {
    try {
      ...
      const patch = {};
      if (body.autoSyncIntervalHours != null) patch.autoSyncIntervalHours = body.autoSyncIntervalHours;
      if (body.deepSleepBeforeOpenHours != null) patch.deepSleepBeforeOpenHours = body.deepSleepBeforeOpenHours;
      if (body.deepSleepAfterCloseHours != null) patch.deepSleepAfterCloseHours = body.deepSleepAfterCloseHours;
      const settings = smarthours.patchSettings(dataDir, patch);
      if (typeof scheduleAutoSync === 'function' && patch.autoSyncIntervalHours != null) scheduleAutoSync();
      res.json({ settings });
    } catch (err) {
  ...
  app.get('/api/smarthours/public/esp32/setup.json', (req, res) => {
    try {
      const eink = require('./smarthoursEink');
      const proto = String(req.get('x-forwarded-proto') || req.protocol || 'https').split(',')[0].trim();
      const host = String(req.get('x-forwarded-host') || req.get('host') || '').split(',')[0].trim();
      const baseUrl = host ? `${proto}://${host}` : '';
      const meta = eink.setupPortalMeta(baseUrl);
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('ETag', `"${meta.setupPageVersion}"`);
      res.json(meta);
    } catch (err) {
      console.error('[SmartHours] setup portal meta error:', err);
      res.status(500).json({ error: err.message });
    }
  });

  app.get('/api/smarthours/public/esp32/setup.html', (req, res) => {
    try {
      const eink = require('./smarthoursEink');
      const html = eink.buildSetupPortalHtml();
      const version = eink.setupPortalVersion();
      const etag = `"${version}"`;
      const inm = String(req.get('If-None-Match') || '').trim();
      res.setHeader('Content-Type', 'text/html; charset=utf-8');
      res.setHeader('Cache-Control', 'no-cache');
      res.setHeader('ETag', etag);
      res.setHeader('X-SmartHours-Setup-Version', version);
      ...
      console.error('[SmartHours] setup portal html error:', err);
      res.status(500).json({ error: err.message });
    }
  });
  ...
  app.get('/api/smarthours/public/:slug/eink.json', (req, res) => {
    try {
      ...
      const host = String(req.get('x-forwarded-host') || req.get('host') || '').split(',')[0].trim();
      const baseUrl = host ? `${proto}://${host}` : '';
      const einkMeta = eink.einkMeta(customer, baseUrl);
      const dispMeta = display.displayMeta(customer, baseUrl);
      // ESP32 firmware reads imageUrl as a 1-bit BMP. Keep PNG on pngUrl so
      // spreading display meta cannot replace the bitmap the panel can paint.
      const meta = {
        ...einkMeta,
        ...dispMeta,
        version: einkMeta.version,
        layoutVersion: einkMeta.layoutVersion,
        openNow: einkMeta.openNow,
        hours: einkMeta.hours,
        inverted: einkMeta.inverted,
        imageUrl: einkMeta.imageUrl,
        bmpUrl: einkMeta.imageUrl,
        openBmpUrl: einkMeta.openBmpUrl,
        closedBmpUrl: einkMeta.closedBmpUrl,
        pngUrl: dispMeta.pngUrl || dispMeta.imageUrl,
        metaUrl: einkMeta.metaUrl,
        setupPageVersion: einkMeta.setupPageVersion,
        setupPageUrl: einkMeta.setupPageUrl
      };
      res.setHeader('Cache-Control', 'no-cache');
  ...
  app.get('/api/smarthours/public/:slug/eink.bmp', (req, res) => {
    try {
      ...
      const openNow = status === 'open' ? true : status === 'closed' ? false : undefined;
      const rendered = eink.renderEinkBmp(customer, { dataDir, openNow });
  ...
  app.post('/api/smarthours/devices/checkin', (req, res) => {
    try {
      ...
      const host = String(req.get('x-forwarded-host') || req.get('host') || '').split(',')[0].trim();
      const baseUrl = host ? `${proto}://${host}` : '';
      const setup = eink.setupPortalMeta(baseUrl);
      res.json({
        ok: true,
        deviceId: device.id,
        ...
        sleepMode: device.sleepMode || 'poll',
        deepSleep: device.deepSleep !== false,
        ip: device.ip || '',
      ...
      notifyLowBatteryAlert(req, device);
    } catch (err) {
  ...
  app.get('/api/smarthours/devices/:id/battery', requireCustomerAccess, (req, res) => {
    try {
      const store = smarthours.loadStore(dataDir);
      ...
      if (!isAdminAuthed(req)) {
        ...
        if (!customer || !smarthours.deviceMatchesSlug(existing, customer.slug)) {
          return res.status(403).json({ error: 'Not allowed to view this device.' });
        }
      }
      const tzOffset = Number(req.query.tzOffset);
      res.json(smarthours.getDeviceBatteryHistory(dataDir, key, tzOffset));
    } catch (err) {
      res.status(err.status || 500).json({ error: err.message });
    }
  });
  ...
  // Auto Google hours sync: per-customer interval (default daily)
  ...
  function scheduleAutoSync() {
    ...
    console.log('[SmartHours] auto sync tick every 1m (per-customer Device Controls Google Hours Sync)');
